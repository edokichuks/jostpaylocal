...
class AddAssetsScreen extends StatefulWidget {
  ...
  AddAssetsScreen({super.key, required this.selectedAccountId});
...
class _AddAssetsScreenState extends State<AddAssetsScreen> {
  ...
  addCustomToken(index, val) async {
    ...
    if (tokenProvider.isTokenAdded == false) {
      ...
      Helper.dialogCall
          .showToast(context, "Token already added to this account");
  ...
  deleteToken(String tokenId) async {
    ...
    await DBTokenProvider.dbTokenProvider.getAccountToken(selectedAccountId);
    var index = DBTokenProvider.dbTokenProvider.tokenList
        .indexWhere((element) => "${element.token_id}" == tokenId);
    ...
    if (index != -1) {
      ...
      // ignore: use_build_context_synchronously
      Navigator.pop(context, "refresh");
      await DBTokenProvider.dbTokenProvider.deleteToken(
          DBTokenProvider.dbTokenProvider.tokenList[index].id.toString());
      DBTokenProvider.dbTokenProvider.tokenList.removeAt(index);
    } else {
      // ignore: use_build_context_synchronously
      Helper.dialogCall
          .showToast(context, "Coin not added in wallet coin list");
      Navigator.pop(context, "refresh");
    }
  }
                        ...
                        child: Row(
                          children: [
                            ...
                            FlutterSwitch(
                              ...
                              value: tokenProvider.selectTokenBool[index]
                                  ["isSelected"],
                              showOnOff: false,
                              onToggle: (val) async {
                                if (val) {
                                  await DbAccountAddress.dbAccountAddress
                                      .getPublicKey(
                                          selectedAccountId, list.networkId);

                                  setState(() {
                                    selectedAccountAddress = DbAccountAddress
                                        .dbAccountAddress
                                        .selectAccountPublicAddress;
