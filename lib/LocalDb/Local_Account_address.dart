...
class DbAccountAddress {
  ...
  Future<int?> deleteAccountAddress(id) async {
    final db = await database;
    final res = await db
        ?.rawDelete("DELETE FROM AccountAddress Where accountId = '$id'");
    return res;
  }

  createAccountAddress(accountId, publicAddress, privateAddress, publicKeyName,
      privateKeyName, networkId, networkName) async {
    final db = await database;
    final res = await db?.rawInsert(
        'INSERT INTO AccountAddress(accountId, publicAddress, privateAddress, publicKeyName, privateKeyName, networkId, networkName) VALUES("$accountId", "$publicAddress", "$privateAddress", "$publicKeyName", "$privateKeyName", "$networkId", "$networkName")');
    return res;
  }
  ...
  getAccountAddress(accountId) async {
    ...
    final res = await db!.rawQuery(
        "SELECT * FROM AccountAddress Where accountId = '$accountId'");

    List<AccountAddress> list =
        res.map((c) => AccountAddress.fromJson(c)).toList();
  ...
  getPublicKey(accountId, networkId) async {
    final db = await database;
    final res = await db!.rawQuery(
        "SELECT publicAddress,privateAddress FROM AccountAddress Where accountId = '$accountId' AND networkId = '$networkId'");
  ...
  getDataByAddress(String address, String networkId) async {
    final db = await database;
    final res = await db!.rawQuery(
        "SELECT privateAddress FROM AccountAddress Where LOWER(publicAddress) = '${address.toLowerCase()}' AND networkId = '$networkId'");
    if (res.isNotEmpty) {
