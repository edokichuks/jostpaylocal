...
//     final res = await db!.update('Token', data,
//         where: "id = ? AND accountId = ? ", whereArgs: [tokenId, id]);
//     getAccountToken(id);
//     return res;
//   }
...
//     final res = await db!.update('Token', data,
//         where: "token_id = ? AND accountId = ? ", whereArgs: [tokenId, id]);
//     getAccountToken(id);
//     return res;
//   }
...
//         "SELECT * FROM Token Where accountId = '$acId' AND market_id='$id'");
...
//     final db = await database;
//     final res =
//         await db!.rawDelete("DELETE FROM Token Where accountId = '$accountId'");
//     return res;
//   }
...
//     final db = await database;
//     final res = await db!
//         .rawQuery("SELECT * FROM Token Where accountId = '$accountId'");
...
//         "UPDATE Token SET price = $livePrice, percent_change_24h = $gainLoss WHERE market_id = '$id'");
...
//     final db = await database;
//     final res = await db!
//         .rawUpdate("UPDATE Token SET balance = $balance WHERE id = '$id'");
...
//     final db = await database;
//     final res =
//         await db!.rawQuery("SELECT price FROM Token Where token_id = '$id'");
//     return res;
//   }
// }
