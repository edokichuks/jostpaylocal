...
class DbSellHistory {
  ...
  updateSellHistory(SellHistoryModel newToken, invoice, id) async {
    ...
    final res = await db!.update('SellHistory', newToken.toJson(),
        where: "invoice = ? AND accountId = ? ", whereArgs: [invoice, id]);
    getSellHistory(id);
    return res;
  }
  ...
  updateStatus(SellHistoryModel newToken, id) async {
    ...
    final res = await db!.update('SellHistory', data,
        where: "invoice = ? AND accountId = ? ",
        whereArgs: [newToken.invoiceNo, id]);
  ...
  getSellHistory(String accountId) async {
    ...
    final res = await db!.rawQuery(
        "SELECT * FROM SellHistory Where accountId = '$accountId' ORDER BY time DESC");

    List<SellHistoryModel> list = res.map((c) {
      return SellHistoryModel.fromJson(c, accountId, c['tokenName'],
          c["accountName"], c['accountNo'], c['bank']);
    }).toList();
  ...
  getTrxStatus(String accountId, String invoiceNo) async {
    final db = await database;
    final res = await db!.rawQuery(
        "SELECT * FROM SellHistory Where accountId = '$accountId' AND invoice = '$invoiceNo'");
